...
func extend_last_point_to(pos: Vector2) -> void:
	...
	walls[p_size-2].shape.size.y += diffl
	walls[p_size-2].position += diff/2
...
func  add_new_point(pos: Vector2, rot: float) -> void:
	...
	wall.shape.size = Vector2(diffl, 0.1)
	wall.position = p1 + diff/2
	wall.rotation = rot
	body.add_child(wall)
...
func handle_length(add_value: float = 0) -> void:
	...
	if l <= shorten_amount or walls[0].shape.size.y <= shorten_amount: #very bad workaround that has to be implemented
		remove_point(0)
	...
	if walls[0].shape.size.y - shorten_amount < 0:
		print(walls[0].shape.size.y)
		
	walls[0].shape.size.y -= shorten_amount
	walls[0].position -= (points[0]-points[1]).normalized()*shorten_amount
